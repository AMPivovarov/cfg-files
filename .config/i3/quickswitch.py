...
def cycle_all_numbered_workspaces(backwards=False, first_last_workspace=(1, 9)):
    '''Get the next (previous) numbered workspace.'''
    current = get_current_workspace()
    if not workspace_is_numbered(current):
        return None
    current = get_workspace_number(current)

    target = current + 1
    if backwards:
        target = current - 1

    if target > first_last_workspace[1]:
        target = first_last_workspace[0]
    elif target < first_last_workspace[0]:
        target = first_last_workspace[1]
...
def main():
    ...
    mutgrp_2.add_argument('-N', '--next-any', type=int, metavar=('FIRST_WORKSPACE', 'LAST_WORKSPACE'), nargs=2,
                          help='go to the next (numbered) workspace, whether it\'s exists or not')
    mutgrp_2.add_argument('-P', '--previous-any', type=int, metavar=('FIRST_WORKSPACE', 'LAST_WORKSPACE'), nargs=2,
